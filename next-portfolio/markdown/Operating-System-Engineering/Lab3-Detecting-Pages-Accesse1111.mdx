...
- The system call handler retrieves the arguments from the appropriate user-space register or memory.
- Here, we need to use function like argaddr() and argint().
        
- **argaddr()**
  - **Purpose:** Retrieves a pointer or memory address.
  - **Return Type:** `uint64` (64-bit).
  - **Common Use Case:** Memory buffer addresses.

- **argint()**
  - **Purpose:** Retrieves an integer value.
  - **Return Type:** `int` (32-bit).
  - **Common Use Case:** Numeric arguments like IDs or flags.

- In user/pgtbltest.c, void `pgaccess_test()`, it calls pgaccess (buf, 32, &abits).
  - the first parameter is `buf`.
  - the second parameter is `32`.
  - the third parameter is `&abits`.
- So in the kernel/sysproc.c, we need to use argaddr() and argint() with this sequence.
...
- In each level page table, if PTE is valid, enter next level page table. If PTE is invalid, alloc it in the memory.
