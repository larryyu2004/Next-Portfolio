...
#### grep fork sh.c | wc -l:
1. The child process creates a pipe to connect the left end of the pipeline with the right end a | b | c:
2. which itself forks two new child processes (one for b and one for c). Thus, the shell may create a tree of processes.
3. The leaves of this tree are commands and the interior nodes are processes that wait until the left and right children complete.
---
## File system
