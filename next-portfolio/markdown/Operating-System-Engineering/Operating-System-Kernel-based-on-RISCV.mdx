...
- User-mode program applies for memory through `malloc`, which maintains a memory pool. When `malloc` cannot provide suitable memory, it will call the `sbrk` system call to request **new heap memory** from the operating system. Under normal circumstances, the `sbrk` system call will mark it in the page table, provide sufficient virtual memory, and apply for sufficient physical memory through `kalloc` to map it to the new virtual memory, and then return false virtual memories for `malloc` to use. The free memory of physical memory is maintained through a linked list, and `kalloc` obtains physical memory from the free linked list.

## What does the stack look like?

- XV-6 allocates two pages of memory for each stack (one page is 4 KB), one page is actually used for the stack, and the other page is used to prevent the stack from overflowing and affecting other memory areas.
- When a function is called, a stack frame will be generated on the stack. Each stack frame stores information such as the return address, registers, and local variables. The stack frame will be popped when the function returns.
- In XV-6, the stack size is fixed at 4KB.

## Do multiple threads use this stack?
- XV-6 does not support multithreading, but in Linux, multiple threads do not share the same stack, and each thread has its own private thread stack.

## Will this stack be recycled after each thread is released?
- Generally speaking, when a thread is released, the thread stack will also be recycled, but you can also set up a thread stack pool so that stack memory can be allocated faster when needed. These thread stacks can be reused and may not be recycled.

## If page faults are triggered frequently, will it affect performance?
- Yes, on Linux, some configuration can be used to reduce its impact on performance.
1. **Adjust Swappiness**: Swappiness is a parameter on Linux that controls the extent to which the Linux system tends to use swap space when memory is insufficient. You can reduce frequent swapping by lowering the Swappiness value, thereby reducing the impact of page faults.
2. **Turn off unnecessary background services**: Stop or disable unnecessary background services and processes to free up memory resources, thereby reducing the chance of page faults.
3. **Adjust Page Size**: If the system supports it, you can try to adjust the page size to match the workload requirements. This requires a deep understanding of the support of the system and hardware, and needs to be handled with caution. The page size is usually configured through `/proc/sys/vm/page_size` or other related parameters.
