...
- Function Call → `this` is `window` (or `undefined` in strict mode)
...
class MyComponent extends Component {
  onClickMe() {
    console.log(this); // ❌ this will be undefined unless bound
  }

  constructor(props) {
    super(props);
    this.onClickMe = this.onClickMe.bind(this); // ✅ Fixes the binding
  }
}
```
...
- ✅ Works without defining parameters
- ❌ Not a real array → can’t use .forEach() or .map()
...
- ✅ You can name the variable (e.g. ...nums)
- ✅ It’s a real array → supports all array methods
...
- Declare a new function → last() to print the last element of the array.k
...
- You cannot call it from an instance like `const p = new Person();` and `p.genericHello()` → that would cause an error.
...
counter(); // 2
counter(); // 3
// Each call “remembers” the `count` variable — that’s closure in action.
```
...
a.includes(3);       // true
a.includes(6);       // false
a.includes(3, 3);    // false → starts checking from index 3 (value is 4)
a.includes(3, 1);    // true  → starts checking from index 1
```
...
a.slice(1, 3);  // [20, 30]
a.slice(2);     // [30, 40, 50]
a.slice();      // [10, 20, 30, 40, 50] → makes a shallow copy
a.slice(-2);    // [40, 50]
```
...
const a = [1, 2, 3, 10, 11];
a.sort(); // → [1, 10, 11, 2, 3]
// Because: '1', '10', '11', '2', '3' → sorted alphabetically


// Mixed types and undefined:
const b = [1, 'a', 'Z', undefined, 3, 2, 11];
b.sort(); // → [1, 11, 2, 3, 'Z', 'a', undefined]
...
1. Capturing phase: The event travels from the top of the DOM (e.g., window → document) down to the target element. You attached a listener to #container with `{ capture: true }`, so it fires during this phase.
2. Target phase: The event reaches the button (the actual target). The button’s listener fires.
...
isItDoneYet
  .then((result) => {
    console.log(result); // → 'Here is the thing I built'
  })
  .catch((error) => {
    console.log(error); // → 'Still working on something else'
  });
...
- Variables declared with `var` are **hoisted**—meaning their declaration is moved to the top of their scope **before** code runs.
...
(function() {
  'use strict'
  variable = 'hey' // ❌ ReferenceError: variable is not defined
})();

(() => {
  'use strict'
  myname = 'Flavio' // ❌ ReferenceError: myname is not defined
})();
...
```JS
undefined = 1;   // 🚫 But JavaScript won't complain
Infinity = 2;
NaN = 3;

// These changes do nothing, but JS won’t tell you — which is risky.
```
...
car.color = 'red'; // ❌ Does nothing in sloppy mode

// In Strict Mode:
'use strict';
car.color = 'yellow'; // ❌ Throws TypeError: Cannot assign to read only property
```
...
car.color = 'red'; // ❌ No setter, but sloppy mode ignores this

// In Strict Mode:
'use strict';
car.color = 'yellow'; // ❌ TypeError: Cannot set property
```
...
car.model = 'Fiesta'; // ❌ Sloppy mode silently fails

// In Strict Mode:
'use strict';
car.owner = 'Flavio'; // ❌ TypeError: Cannot add property to non-extensible object
```
...
1.name = 'xxx';       // ❌ TypeError
'test'.testing = true; // ❌ TypeError
```
...
- deep = `false` → copies only the current element (no children)
- deep = `true` → copies the element and all child nodes
...
| `toggle(class)`      | Adds the class if it doesn’t exist; removes it if it does.                    |
...
- Example: Clicking an inner `<div>` triggers its click handler, and also the handlers of its parent `<div>`s—unless bubbling is stopped.
